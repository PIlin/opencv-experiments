...
std::unique_ptr<MarkerDetectionFacade> createMarkerDetection(CameraCalibration calibration)
{
  //return std::auto_ptr<MarkerDetectionFacade>(new MarkerDetector(calibration));
  return make_unique<MarkerDetector>(calibration);
}
...
void MarkerDetector::processFrame(const BGRAVideoFrame& frame)
{
  ...
  for (auto m: markers)
  {
    m_transformations.push_back(m.transformation);
  }
}
